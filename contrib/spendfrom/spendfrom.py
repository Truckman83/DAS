...
def determine_db_dir():
    """Return the default location of the Das Core data directory"""
    if platform.system() == "Darwin":
        return os.path.expanduser("~/Library/Application Support/DasCore/")
    elif platform.system() == "Windows":
        return os.path.join(os.environ['APPDATA'], "DasCore")
    return os.path.expanduser("~/.dascore")
...
def connect_JSON(config):
    """Connect to a Das Core JSON-RPC server"""
...
def main():
    ...
    parser.add_option("--datadir", dest="datadir", default=determine_db_dir(),
                      help="location of das.conf file with RPC username/password (default: %default)")
    ...
    dasd = connect_JSON(config)

    if options.amount is None:
        address_summary = list_available(dasd)
        ...
        while unlock_wallet(dasd) == False:
            pass # Keep asking for passphrase until they get it right
        txdata = create_tx(dasd, options.fromaddresses.split(","), options.to, amount, fee)
        sanity_test_fee(dasd, txdata, amount*Decimal("0.01"))
        if options.dry_run:
            print(txdata)
        else:
            txid = dasd.sendrawtransaction(txdata)
            print(txid)
